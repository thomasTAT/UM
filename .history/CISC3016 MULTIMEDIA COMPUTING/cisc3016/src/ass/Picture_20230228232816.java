...
public class Picture extends SimplePicture {
	...
	public Picture copyPicture(int init_x, int init_y,int x, int y) {
		Pixel sourcePixel = null;
		Pixel targetPixel = null;
		Picture picture = new Picture(x-init_x,y-init_y);

		for (int sourceX = init_x, targetX = 0; sourceX < x; sourceX++, targetX++) {
			// loop through the rows
			for (int sourceY = init_y, targetY = 0; sourceY < y; sourceY++, targetY++) {
				sourcePixel = this.getPixel(sourceX, sourceY);
				targetPixel = picture.getPixel(targetX, targetY);
				targetPixel.setColor(sourcePixel.getColor());
			}
